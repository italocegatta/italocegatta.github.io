...
Ajustar um modelo linear ou não linear é algo relativamente simples no R. Mas em muitos casos precisamos ajustá-lo para vários fatores e dependendo da qunatidade isso se torna uma tarefa chata. Se você, assim como eu, já precisou fazer isso no Excel, sabe o que é perder mais que uma tarde copiando e colando informações entres abas e planilhas.   
<!-- more -->
...
```{r}
library(pacman)
p_load(readr, dplyr, tidyr, broom, purrr, ggplot2)
```

```{r, message=FALSE}
dados <- read_csv2("https://goo.gl/e8aWm0")
...
```{r 9-dap-h, fig.cap="Relação entre o diâmetro e a altura sem destinção de espécie."}
